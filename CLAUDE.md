...
3. セッション再開失敗時やBot再起動後 → スレッド履歴から会話コンテキストを再構築し新規セッションで応答
4. スレッドごとにロック (`thread_locks`) で直列化し、同一スレッドへの並行応答を防止
5. `active_sessions` でBot参加済みスレッドを管理。再起動後はスレッド履歴からBotの参加を検出して復元
